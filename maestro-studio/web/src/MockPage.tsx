    ...
    <div className="flex flex-col px-8 max-h-full overflow-hidden">
      {data?.events.length === 0 && !!data.projectId ? (
        <div className="flex flex-col w-3/4 space-y-4 pt-4">
          ...
          <CodeSnippet>{`MaestroSDK.init('${data.projectId}')`}</CodeSnippet>

          <p className="text-md">Lastly, retrieve your API url from Maestro SDK:</p>
          ...
          <p className="font-semibold">Build and run your app and you should start seeing events come in here!</p>
        </div>
      ) : (
        <>
