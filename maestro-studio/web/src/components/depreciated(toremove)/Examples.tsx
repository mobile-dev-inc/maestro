...
export const CodeSnippet = ({ children }: { children: string }) => {
  ...
  return (
    <CopyToClipboard text={children} onCopy={() => setCopied(true)}>
      ...
      >
        <pre className="overflow-x-scroll p-5">{children}</pre>
        {hovered ? (
          <div className="absolute top-0 right-0 flex">
            <span className="px-2 py-1 font-sans text-slate-400 dark:text-white">
...
const Section = ({
  ...
  const codeSnippetComponents = codeSnippets
    .map((codeSnippet) => {
      ...
      if (
        codeSnippet.includes("[resource-id-index]") &&
        !elementHasResourceIdIndex
      )
        return null;
      ...
      if (
        codeSnippet.includes("[accessibilityText]") &&
      ...
      if (codeSnippet.includes("[text-index]") && !elementHasTextIndex)
        return null;
      ...
      if (
        elementHasTextIndex &&
        codeSnippet.includes("[text]") &&
        !codeSnippet.includes("[text-index]")
      )
        return null;
...
const Error = ({ element }: { element: UIElement }) => {
  const content = (() => {
    if (isBoundsEmpty(element)) {
      ...
      return (
        <span>
          This element's <b>width or height is 0</b> so you won't be able to tap
          on it. If you believe this is a problem with Maestro, please{" "}
          {openAGitHubIssueLink} and include a screenshot of this page.
        </span>
      );
    }
    if (!element.text && !element.resourceId) {
      return (
        <span>
          This element does not have any <b>text</b> or <b>resource id</b>{" "}
          associated with it. You can still tap on this element using screen
          percentages, but this won't be as resilient to changes to your UI.
        </span>
      );
    }
  })();
  if (!content) return null;
  return <div className="bg-red-100 rounded-md p-5">{content}</div>;
};
