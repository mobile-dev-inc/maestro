...
const Footer = ({
  ...
  const content = selectedElement ? (
    <span className="whitespace-nowrap">
      Click on an example to copy the command
    </span>
  ) : hoveredElement ? (
    <>
    ...
    <span className="whitespace-nowrap">
      Click on an element in the screenshot or search for an element by text or
      ID
    </span>
  );
  return (
...
const Inspect = ({
  ...
  const selectedElement =
    deviceScreen.elements.find((e) => e.id === selectedElementId) || null;
  ...
  return (
    <motion.div className="flex flex-col overflow-hidden justify-end h-full dark:bg-slate-800">
      <motion.div
