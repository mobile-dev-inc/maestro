...
- **`test-start-device` job** reads `maestro-android/supported-apis.properties` (the `range` job turns it into a matrix) and runs `maestro start-device --platform android --device-model <device> --device-os android-<N>` for every level from `min` to `max`. It takes no workflow input: it tests the driver's claim, through the CLI's own provisioning path that `test-android` bypasses. Part of `e2e-gate`.
