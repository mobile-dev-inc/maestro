...
| Module                       | Role                                                                                                                                                                                                                                                                                     |
|------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `maestro-android/`           | On-device Android driver. Kotlin sources compile to two checked-in APKs (`maestro-app.apk`, `maestro-server.apk`) consumed by `maestro-client/`. The build's `copyMaestroAndroid` / `copyMaestroServer` finalizers update those APKs plus a `maestro-android-source.sha256` checksum. Its supported API range is declared in the repo-root `supported-platforms.properties` (`android.min=`/`android.max=`); `android.max` trails `compileSdk` and moves only after the e2e corpus is green at that API (see `bump-android-version`).    |
...
- **`workflow_dispatch` inputs** — `android_version` (string, `android-<N>`, default `android-32`), `app` (string, default `demo_app`), `flow` (string, optional single-flow). The `validate-inputs` job rejects `android_version` below `android.min` in `supported-platforms.properties` (no ceiling: a bump validates `max+1` before advancing the file), missing `app` workspace, or ambiguous `flow`. (See PR #3226.)
...
- **`test-start-device` job** reads `android.max` from `supported-platforms.properties` (the driver's declared API range) and runs `maestro start-device --platform android --device-os android-<max>` for each device profile in its matrix. It takes no workflow input: it tests the driver's claim, through the CLI's own provisioning path that `test-android` bypasses. Part of `e2e-gate`.
...
Skills that bump platform versions (Android API levels, iOS versions) drive this workflow via `gh workflow run test-e2e.yaml --ref <branch> -f android_version=<...>`. `bump-android-version` also advances `supported-platforms.properties` once the corpus is green, which is what the copilot device-readiness harness reads through the `maestro` submodule to keep the device catalog from listing an OS above what the driver supports.
