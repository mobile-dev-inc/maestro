...
- **`test-start-device` job** reads `max` from `maestro-android/supported-apis.properties` (the driver's declared API range) and runs `maestro start-device --platform android --device-os android-<max>` for each device profile in its matrix. It takes no workflow input: it tests the driver's claim, through the CLI's own provisioning path that `test-android` bypasses. Part of `e2e-gate`.
